...
import {
  ...
  UserCheck, RefreshCw, Calendar, Clock, Ban, Phone, Pencil, IndianRupee,
} from 'lucide-react';
...
export default function BookingManagement() {
  ...
  const [updateData, setUpdateData]           = useState({
    ...
    customerPhone: '', serviceLabel: '', totalAmount: '',
    cancellationReason: '',
  });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (updateData.customerPhone.trim() !== selectedBooking.customerPhone)
        payload.customerPhone = updateData.customerPhone.trim();

      if (updateData.serviceLabel.trim() !== selectedBooking.serviceLabel)
        payload.serviceLabel = updateData.serviceLabel.trim();

      const newAmount = Number(updateData.totalAmount);
      if (!isNaN(newAmount) && newAmount !== selectedBooking.totalAmount)
        payload.totalAmount = newAmount;

      if (updateData.cancellationReason !== (selectedBooking.cancellationReason || ''))
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {bookings.map((b) => (
                <div key={b._id} className={`p-4 ${isCancelledTab ? 'hover:bg-red-50/30' : 'hover:bg-gray-50'} transition-colors`}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      <div className={`font-mono text-xs font-semibold ${isCancelledTab ? 'text-red-500' : 'text-primary-600'}`}>{b.bookingId}</div>
                      <div className="font-medium text-gray-900 mt-0.5">{b.customerName}</div>
                      <a href={`tel:${b.customerPhone}`} className="flex items-center gap-1 text-xs text-gray-500 mt-0.5">
                        <Phone className="w-3 h-3" />{b.customerPhone}
                      </a>
                    </div>
                    <div className="flex flex-col items-end gap-1.5">
                      {!isCancelledTab && <span className={`badge-${b.status}`}>{STATUS_CONFIG[b.status]?.label}</span>}
                      <button
                        onClick={() => openModal(b)}
                      ...
                      >
                        {isCancelledTab ? 'View' : 'Edit'}
                      </button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-500 mt-2">
                    <div><span className="text-gray-400">Service: </span>{b.serviceLabel}</div>
                    <div><span className="text-gray-400">Amount: </span><span className="font-semibold text-gray-700">{formatCurrency(b.totalAmount)}</span></div>
                    <div><span className="text-gray-400">Date: </span>{formatDate(b.scheduledDate)}</div>
                    <div><span className="text-gray-400">Slot: </span>{b.timeSlot}</div>
                    {!isCancelledTab && (
                      <div className="col-span-2">
                        <span className="text-gray-400">Staff: </span>
                        {b.assignedStaff ? b.assignedStaff.name : <span className="text-gray-300">Unassigned</span>}
                      </div>
                    )}
                    {isCancelledTab && b.cancellationReason && (
                      <div className="col-span-2 text-red-500">{b.cancellationReason}</div>
                    )}
                  </div>
                </div>
              ))}
                ...
                <tbody>
                  {bookings.map((b) => isCancelledTab ? (
                    <tr key={b._id} className="border-b border-gray-50 hover:bg-red-50/40 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="text-gray-700">{b.serviceLabel}</span></td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => openModal(b)} className="text-xs text-primary-600 hover:underline font-medium px-2 py-1 rounded hover:bg-primary-50 transition-colors">View</button>
                      </td>
                    </tr>
                  ) : (
                    <tr key={b._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => openModal(b)} className="text-xs text-primary-600 hover:underline font-medium px-2 py-1 rounded hover:bg-primary-50 transition-colors">Edit</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-400">{formatDate(selectedBooking.createdAt)}</span>
                </div>
              ...
              {/* ── Editable customer & booking details ── */}
              {!isCancelledTab && (
                <div className="border border-blue-200 rounded-xl p-4 space-y-3 bg-blue-50/30">
                  <p className="text-xs font-semibold text-blue-700 flex items-center gap-1.5">
                    <Pencil className="w-3.5 h-3.5" /> Edit Booking Details
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="label text-xs flex items-center gap-1"><Phone className="w-3 h-3" /> Customer Phone</label>
                      <input
                        type="tel"
                        className="input-field text-sm"
                        value={updateData.customerPhone}
                        onChange={(e) => setUpdateData((d) => ({ ...d, customerPhone: e.target.value }))}
                        placeholder="Customer phone"
                        maxLength={10}
                      />
                    </div>
                    <div>
                      <label className="label text-xs flex items-center gap-1"><IndianRupee className="w-3 h-3" /> Amount (₹)</label>
                      <input
                        type="number"
                        className="input-field text-sm"
                        value={updateData.totalAmount}
                        onChange={(e) => setUpdateData((d) => ({ ...d, totalAmount: e.target.value }))}
                        placeholder="0"
                        min={0}
                      />
                    </div>
                    <div className="sm:col-span-2">
                      ...
                      <input
                        type="text"
                        className="input-field text-sm"
                        value={updateData.serviceLabel}
                        onChange={(e) => setUpdateData((d) => ({ ...d, serviceLabel: e.target.value }))}
              ...
              {!isCancelledTab && (
                <div className="space-y-3">
                  ...
                  {updateData.status === 'cancelled' && (
                    <div>
                      <label className="label text-xs text-red-600">Cancellation Reason</label>
                      <input
                        type="text"
                        className="input-field text-sm border-red-200 focus:border-red-400"
                        placeholder="e.g. Customer requested, rescheduled, etc."
                        value={updateData.cancellationReason}
                        onChange={(e) => setUpdateData((d) => ({ ...d, cancellationReason: e.target.value }))}
                      />
                    </div>
                  )}
                </div>
